...
2. **📊 10M+ Card Catalog** — the structured knowledge layer behind identification, spanning baseball, basketball, football, hockey, and Pokémon.
...
- **99.5% accuracy across 3,000+ sets** spanning baseball, basketball, football, and hockey
- **Sub-second response times** — built for real-time camera flows, not batch jobs
- **Multi-card detection** — identify multiple cards in a single image, including binder pages and showcase shots
- **Slab / grading awareness** — detects graded cards and reads the slab (PSA, BGS, SGC, TAG) including grade value, condition, qualifiers, and autograph grades
...
- **Works with messy reality** — poor lighting, angles, reflections, phone cameras, multiple cards in frame

**What you can build with it:**

- **Scan-to-list flows** for marketplaces that eliminate manual data entry
- **Collection apps** where users snap a photo and the card is fully catalogued
- **Grading and pre-screening tools** that recognize slabs and grade values automatically
- **AI assistants** that can "see" a card the user holds up and answer questions about it
...
// Target a specific sport for faster, more accurate results
const footballResult = await client.identify.cardBySegment('football', imageFile);
...
- **Bulk pricing** — pull data for up to 100 cards in a single call
...
Identification and pricing only work because of the catalog underneath. Ten million cards, fully structured, with parallels, variations, attributes, grading references, and card images.

- **10M+ cards** from 1934 through present
- **Baseball, basketball, football, hockey, and Pokémon** — with card images for Pokémon already in the catalog
...
|---|---|
| **🔍 Visual Card Identification** | 99.5% accuracy across 3,000+ sets, sub-second response, handles raw cards and graded slabs |
| **🏷️ Slab & Parallel Awareness** | Automatic grading company / grade detection and parallel variant recognition |
| **🖼️ Multi-Card Detection** | Identify multiple cards in a single image |
...
| **📊 10M+ Card Catalog** | Comprehensive metadata for baseball, basketball, football, hockey, and Pokémon |
...
That's **3,000+ sets** across the four major North American sports, all identifiable from a single photo.

**Catalog is live for:**

- All four sports above, plus 🎮 **Pokémon** — TCG cards with full metadata and card images are in the catalog now

**Coming soon:**

- 🎮 **Pokémon identification** — visual ID for Pokémon TCG cards is next on the roadmap
...
**Scan-to-list marketplaces** — users photograph cards, your listings populate automatically. Operators report listing abandonment drops of up to 70% when sellers don't have to hand-enter card details.

**Collection and portfolio apps** — users snap a photo, your app catalogues and values the card instantly, with daily-refreshed valuations across their entire collection.

**Card shop POS and inventory tools** — identify cards at the counter, price them against live market data, track inventory against the catalog.
...
- 10M-card catalog with parallels, variations, and attributes — 6–12 months, $250K+
